...
class VLLMGen:
    def __init__(self, endpoint, port, tokenizer=None, modelname=None):
        self.openai_api_base = f"http://{endpoint}:{port}/v1"
        ...
        self.vllm_server = VLLM(modelname, port)
        self.port = port
        self.modelname = modelname

    def _postprocess_response(self, response, tags=["[Start of Response]", "[End of Response]"]):
        start_tag, end_tag = map(re.escape, tags)  # Escape special characters in tags
        pattern = rf"{start_tag}(.*?){end_tag}"

        match = re.search(pattern, response, re.DOTALL)
        return match.group(1).strip() if match else ""

    def generate(self, messages, temperature=0.5, parse_response=False, continue_assistant_turn=False, repetition_penalty=1.2):
        ...
        if continue_assistant_turn:
            ...
            completion = self.vllm_server.make_vllm_request(
                [prompt],
                port=self.port,
